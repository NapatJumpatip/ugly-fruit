...
with st.sidebar:
    ...
    "Select Fruit Classes", 
    ["orange", "apple", "banana"], 
    default=["orange", "apple", "banana"] # ให้มันเลือกไว้ให้เลยทั้ง 3 อย่างตอนเปิดแอป
    )
...
def process_image(img_bgr):
    ...
    for i, det in enumerate(detections.boxes.data.tolist()):
        ...
        if class_name not in selected_fruits:   
            continue
        ...
        label = f"{class_name} {confidence:.0%}" if show_conf else class_name
...
for file_idx, uploaded in enumerate(uploaded_files):
    ...
    with st.expander(f"🖼️ ภาพที่ {file_idx+1} — {uploaded.name}", expanded=True):
        ...
        if counts["total"] == 0:
            st.warning(f"ไม่พบ {selected_fruits} — ลองลด Confidence Threshold ใน sidebar ค่ะ")
            continue
        ...
        for det in results_raw.boxes.data.tolist():
            xmin, ymin, xmax, ymax, confidence, class_id = det[:6]
            detected_name = yolo_model.names[int(class_id)]
            if detected_name not in selected_fruits:
                continue
